...
import { createGetCitiesFromMunicipalityHandler } from "../../src/controllers/municipalityController";
import assert from "node:assert";
import { validate } from "../../src/middleware/validateInputMiddleware";
import { getCitiesFromMunicipalityValidation } from "../../src/validation/municipalityValidation";
import { createResponse } from "../helpers/createResponse";

describe("GET cities from municipality test", () => {
  ...
  for (const invalidId of invalidIds) {
    it(`rejects invalid id "${invalidId}"`, async () => {
      ...
      async function fakeGetCitiesFromMunicipality() {
        serviceCallCount++;
        return [];
      }

      const handler = createGetCitiesFromMunicipalityHandler(
        fakeGetCitiesFromMunicipality,
      );
      const middleware = validate(getCitiesFromMunicipalityValidation);
  ...
  it("returns 200 when no cities are attached to a municipality", async () => {
    let serviceCallCount = 0;

    async function fakeGetCitiesFromMunicipality(id: number) {
      serviceCallCount++;
    ...
    const middleware = validate(getCitiesFromMunicipalityValidation);
    const { response, getBody, wasStatusCalled, wasJsonCalled } =
      createResponse();
